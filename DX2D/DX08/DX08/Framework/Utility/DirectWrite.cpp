...
DirectWrite::DirectWrite()
{
	...
	ComPtr<IDXGIDevice> dxgiDevice; assert(SUCCEEDED(DEVICE->QueryInterface(dxgiDevice.GetAddressOf())));
	_factory->CreateDevice(dxgiDevice.Get(), _device.GetAddressOf());
	ASSERT(_device->CreateDeviceContext(D2D1_DEVICE_CONTEXT_OPTIONS_ENABLE_MULTITHREADED_OPTIMIZATIONS, _dc.GetAddressOf()));
	...
	assert(SUCCEEDED(_dc->CreateBitmapFromDxgiSurface(dxgiSurface.Get(), &bp, _targetBitmap.GetAddressOf())));
...
void DirectWrite::RenderText(wstring text, RECT rect, float size, wstring fontPath, XMFLOAT4 color, DWRITE_FONT_WEIGHT weight, DWRITE_FONT_STYLE style, DWRITE_FONT_STRETCH stretch)
{
	...
	if (findText == nullptr)
	{
		ASSERT(_writeFactory->CreateTextFormat(fontDesc._fontPath.c_str(), nullptr, fontDesc._weight,
			fontDesc._style, fontDesc._stretch, fontDesc._fontSize, L"ko", fontDesc._format.GetAddressOf()));
